...
app.post("/create", (req, res) => {
  ...
  connection.query(
    "INSERT INTO aspirantes(nombre_aspirante, codigo_carnet, email_aspirante, telefono_aspirante, id_periodo,id_entrevistador, id_programa ) VALUES (?, ?, ?, ?, ?, ?, ?)",
    [
      nombre_aspirante,
      codigo_carnet,
      email_aspirante,
...
app.get("/aspirantes", (req, res) => {
  connection.query(
    "SELECT aspirantes.*, nombre_entrevistador, periodos.año, periodos.semestre, programa.nombre_programa  FROM aspirantes INNER JOIN entrevistador ON aspirantes.id_entrevistador = entrevistador.id_entrevistador INNER JOIN periodos ON aspirantes.id_periodo = periodos.id_periodo INNER JOIN programa ON aspirantes.id_programa = programa.id_programa; ",
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.get("/aspirantes/:id_aspirante", (req, res) => {
  ...
  connection.query(
    "SELECT * FROM aspirantes WHERE id_aspirante = ?",
    id_aspirante,
    (err, result) => {
      if (err) {
        console.log(err);
...
app.put("/update", (req, res) => {
  ...
  connection.query(
    "UPDATE aspirantes SET nombre_aspirante= ?, codigo_carnet= ?, email_aspirante= ?, telefono_aspirante= ?, id_entrevistador= ?, id_periodo= ?, id_programa= ? WHERE id_aspirante= ?",
    [
      nombre_aspirante,
      codigo_carnet,
      email_aspirante,
...
app.delete("/delete/:id_aspirante", (req, res) => {
  ...
  connection.query(
    "DELETE FROM aspirantes WHERE id_aspirante= ?",
    id_aspirante,
    (err, result) => {
      if (err) {
        console.log(err);
...
app.get("/periodo", (req, res) => {
  connection.query("SELECT * FROM periodos", (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
...
app.get("/entrevistador", (req, res) => {
  connection.query("SELECT * FROM entrevistador", (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
...
app.get("/criterios", (req, res) => {
  connection.query("SELECT * FROM criterios", (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
...
app.get("/sub_criterios", (req, res) => {
  connection.query("SELECT * FROM sub_criterios", (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
...
app.post("/Entrevista", (req, res) => {
  ...
  Object.entries(notas_sub_criterios).forEach(
...
app.get("/informacion_basica/:id_aspirante", (req, res) => {
  ...
  connection.query(
    "SELECT * FROM informacion_basica WHERE id_aspirante = ?",
    [id_aspirante],
    (err, result) => {
      if (err) {
        console.error(
          "Error al obtener la información básica del aspirante:",
          err
        );
        res
          .status(500)
          .json({
            error: "Error al obtener la información básica del aspirante",
          });
      } else {
...
// Ruta para guardar o actualizar datos en la tabla informacion_basica
app.post("/informacion_basica/:id_aspirante", async (req, res) => {
  const { id_aspirante, nota_general } = req.body;
  ...
  try {
    ...
    if (result.length === 0) {
      const insertResult = await connection.query(
        "INSERT INTO informacion_basica (id_aspirante, nota_general) VALUES (?, ?)",
        [id_aspirante, nota_general]
      );
      return res.status(201).json(insertResult);
    } else {
      const updateResult = await connection.query(
        "UPDATE informacion_basica SET nota_general = ? WHERE id_aspirante = ?",
        [nota_general, id_aspirante]
      );
...
// Actualiza la información básica del aspirante si ya existe el id_aspirante
app.put("/informacion_basica/:id_aspirante", (req, res) => {
  const id_aspirante = req.params.id_aspirante;
  const { nota_general } = req.body; // Ajusta esto según la estructura de los datos que recibas del frontend

  // Realiza la actualización en la tabla informacion_basica
  connection.query(
    "UPDATE informacion_basica SET nota_general = ? WHERE id_aspirante = ?",
    [nota_general, id_aspirante], // Ajusta los campos según la estructura de tu tabla
    (err, result) => {
      if (err) {
        console.error(
          "Error al actualizar la información básica del aspirante:",
          err
        );
        res
          .status(500)
          .send("Error al actualizar la información básica del aspirante.");
      } else {
        if (result.affectedRows === 0) {
          res
            .status(404)
            .send("No se encontró ningún aspirante con el ID proporcionado.");
        } else {
          res.send(result);
        }
      }
...
app.get("/sub_criterios-notas", (req, res) => {
  ...
  connection.query(
    "SELECT * FROM sub_criterio_aspirante WHERE id_aspirante = ?",
    [id_aspirante],
    (err, result) => {
      if (err) {
        console.log(err);
...
app.post("/guarda_porcentaje", (req, res) => {
  ...
  const id_aspirante = req.body.id_aspirante;
  connection.query(
    "INSERT INTO probabilidad(probabilidad,id_aspirante ) VALUES (?, ?)",
    [probabilidad, id_aspirante],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.get("/obtener-probabilidad-por-aspirante/:id", (req, res) => {
  const id_aspirante = req.params.id;
  console.log(id_aspirante)
  connection.query('SELECT probabilidad FROM probabilidad WHERE id_aspirante = ?', [id_aspirante], 
    (err, result) => {
      if (err) {
        console.log(err);
        ...
        if (result.length > 0) {
          const probabilidad = result[0].probabilidad;
          res.status(200).json({ probabilidad });
        } else {
          res.status(404).send("Aspirante no encontrado");
        }
      }
    }
  )
...
app.post("/createcriterio", (req, res) => {
  ...
  connection.query(
    "INSERT INTO criterios(nombre_criterio, porcentaje_criterio) VALUES (?, ?)",
    [nombre_criterio, porcentaje_criterio],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.post("/createsubcriterio", (req, res) => {
  ...
  connection.query(
    "INSERT INTO sub_criterios(nombre_sub_criterio, descripcion_sub_criterio,id_criterio, nota_minima, nota_maxima) VALUES (?,?,?,?,?)",
    [
      nombre_sub_criterio,
      descripcion_sub_criterio,
      id_criterio,
      nota_minima,
      nota_maxima,
    ],
...
app.delete("/deletesub-criterio/:id_sub_criterio", (req, res) => {
  ...
  connection.query(
    "DELETE FROM sub_criterios where id_sub_criterio = ?",
    id_sub_criterio,
    (err, result) => {
      if (err) {
        // Registrar el error para depuración
        ...
        // Registrar éxito para depuración
        console.log(
          `Sub-criterio con ID: ${id_sub_criterio} eliminado exitosamente`
        );
...
app.delete("/deletecritero/:id_criterio", (req, res) => {
  ...
  connection.query(
    "DELETE FROM criterios WHERE id_criterio = ?",
    id_criterio,
    (err, result) => {
      if (err) {
        console.log(err);
...
app.get("/obtener_porcentaje", (req, res) => {
  ...
  console.log("ID del aspirante recibido:", id_aspirante);
  connection.query(
    "SELECT * FROM sub_criterio_aspirante WHERE sub_criterio_aspirante.id_aspirante =?",
    [id_aspirante],
    (err, result) => {
      if (err) {
        console.log(err);
  ...
  connection.query(
    ...
    (err, result) => {
      if (err) {
        ...
        res
          .status(500)
          .json({ error: "Error al obtener la entrevista del aspirante" });
      } else {
        res.json(result); // Devuelve el resultado de la consulta como JSON
      }
    }
...
app.put("/update_nota", (req, res) => {
  ...
  connection.query(
    "UPDATE sub_criterio_aspirante SET nota_sub_criterio_aspirante = ? WHERE id_sub_criterio = ? AND id_aspirante = ?;",
    [nota_sub_criterio_aspirante, id_sub_criterio, id_aspirante],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.delete("/eliminar_subcriterios_aspirante/:idAspirante", (req, res) => {
  ...
  connection.query(
    "DELETE FROM sub_criterio_aspirante WHERE id_aspirante = ?",
    [idAspirante],
    (err, result) => {
      if (err) {
        console.log(err);
        res
          .status(500)
          .send("Error al eliminar registros en sub_criterio_aspirante");
      } else {
        res.send(result);
      }
    }
...
app.post("/createperiodo", (req, res) => {
  ...
  connection.query(
    "INSERT INTO periodos(año, semestre) VALUES (?, ?)",
    [año, semestre],
    (err, result) => {
      if (err) {
        console.log(err);
